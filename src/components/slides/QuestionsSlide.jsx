      ...
      <div className="flex flex-col items-center space-y-12">
        ...
        <div className="w-full grid md:grid-cols-2 gap-8 text-left">
          {questions.map((q, i) => (
            <div
              key={q.id}
              className="bg-white border border-[#E7E9E1] rounded-[24px] p-8 space-y-4"
            >
              <span className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-[#DDD6FE] text-[#1F2937] text-sm font-bold">
                {i + 1}
              </span>
              <h3 className="text-lg font-medium text-[#1F2937] leading-snug">
                {q.text}
              </h3>
            </div>
          ))}
